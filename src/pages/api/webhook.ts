 ...
 * from the private R2 bucket without ever exposing the bucket itself.
 */
export const POST: APIRoute = async ({ request, locals }) => {
	...
	console.log(
		`Buzzyfly webhook: fulfilled order ${order.orderId} (${order.itemId}) for ${order.customerEmail ?? "unknown email"} -> ${downloadUrl}`,
	);

	return new Response(
		JSON.stringify({ received: true, fulfilled: true, downloadUrl }),
